...
DOMAIN-scoped retrofit of routers/delegation_policy.py's own
establish_delegation_policy (BA-04, TD-024), moved for scope_type =
...
(TD-022) and establish_approval_authority's own DOMAIN-scoped subset
(TD-023).

ORGANIZATION, OBJECT, and EVENT scope remain PLATFORM_ADMIN-only,
unchanged — Corporate Admin authority (URA-001-32) is not modeled
anywhere in this repository, and no authority is named anywhere for
OBJECT or EVENT scope; retrofitting either would require inventing new
architecture, which this Work Package does not do.
...
async def two_orgs(db_session: AsyncSession) -> dict:
    ...
    role = Role(role_code="DP_RETROFIT_TEST_ROLE", role_name="Delegation Policy Retrofit Test Role")
    db_session.add(role)
    await db_session.flush()

    org_a = Organization(organization_code="DELPOL-RETROFIT-ORG-A", organization_name="Retrofit Org A", organization_type="CORPORATE")
    ...
    org_b = Organization(organization_code="DELPOL-RETROFIT-ORG-B", organization_name="Retrofit Org B", organization_type="CORPORATE")
...
async def test_establish_delegation_policy_domain_scope_succeeds_with_admin_grant(
...
async def test_establish_delegation_policy_domain_scope_denies_insufficient_grant(
    client: TestClient, db_session: AsyncSession, two_orgs: dict
) -> None:
...
async def test_establish_delegation_policy_domain_scope_denies_no_grant(
    client: TestClient, db_session: AsyncSession, two_orgs: dict
) -> None:
    ctx = two_orgs
    headers = _headers_for(ctx, "a")
...
async def test_establish_delegation_policy_domain_scope_denies_grant_from_unrelated_organization(
    client: TestClient, db_session: AsyncSession, two_orgs: dict
) -> None:
    """
    ...
    establishing a DOMAIN-scoped Delegation Policy against Domain A —
...
async def test_establish_delegation_policy_domain_scope_succeeds_for_platform_admin(
    client: TestClient, db_session: AsyncSession, two_orgs: dict
) -> None:
...
# ---------------------------------------------------------------------------
# scope_type = ORGANIZATION / OBJECT / EVENT — existing PLATFORM_ADMIN-only
# behavior preserved; a DOMAIN-scoped ADMIN grant must not leak into them.
# ---------------------------------------------------------------------------

async def test_establish_delegation_policy_organization_scope_denies_domain_admin_grant_holder(
    client: TestClient, db_session: AsyncSession, two_orgs: dict
) -> None:
    """A caller's own DOMAIN-scoped ADMIN grant must not satisfy ORGANIZATION scope's own unchanged PLATFORM_ADMIN-only gate."""
    ctx = two_orgs
    await _grant(db_session, ctx["membership_a"], ctx["domain_a"], "ADMIN")
    headers = _headers_for(ctx, "a")
...
async def test_establish_delegation_policy_organization_scope_succeeds_for_platform_admin(
    client: TestClient, db_session: AsyncSession, two_orgs: dict
) -> None:
    ctx = two_orgs
    headers = _headers_for(ctx, "a", role_code="PLATFORM_ADMIN")
...
async def test_establish_delegation_policy_object_scope_denies_domain_admin_grant_holder(
    client: TestClient, db_session: AsyncSession, two_orgs: dict
) -> None:
    ctx = two_orgs
    await _grant(db_session, ctx["membership_a"], ctx["domain_a"], "ADMIN")
    ...
    response = client.post(
        ...
        json={
            "organization_id": str(ctx["org_a"].id),
            "policy_name": "Revenue CDE Delegation",
...
async def test_establish_delegation_policy_object_scope_succeeds_for_platform_admin(
    client: TestClient, db_session: AsyncSession, two_orgs: dict
) -> None:
    ctx = two_orgs
    headers = _headers_for(ctx, "a", role_code="PLATFORM_ADMIN")
...
async def test_establish_delegation_policy_event_scope_denies_domain_admin_grant_holder(
    client: TestClient, db_session: AsyncSession, two_orgs: dict
) -> None:
    ctx = two_orgs
    await _grant(db_session, ctx["membership_a"], ctx["domain_a"], "ADMIN")
...
async def test_establish_delegation_policy_event_scope_succeeds_for_platform_admin(
    client: TestClient, db_session: AsyncSession, two_orgs: dict
) -> None:
    ctx = two_orgs
    headers = _headers_for(ctx, "a", role_code="PLATFORM_ADMIN")
