...
| BA-04 | Shape / Refine Proposed Structural Outcome | Create / Update (proposal) | Proposed Outcome Context — **v1 scoped to EnterpriseNode-targeted proposals only, per ADR-007**; EnterpriseRelationship/ConsolidationDetermination-targeted proposals explicitly deferred, not eliminated (ADR-007 point 3) | ERB-C005-04 / EX-C005-05, -06 | ⏳ Not started — target-type ambiguity resolved (ADR-007); own implementation-readiness gap analysis still required |
...
| BA-04 — Shape/Refine Proposed Structural Outcome | **C** (Architecture requires completion — implementation-level; downgraded from D) | **Target-type ambiguity resolved (ADR-007):** BA-04 v1 is scoped to EnterpriseNode-targeted proposals only — a phased implementation decision, not an architectural one; ERG-001 and SCI-000001's own registration (§21) are both unamended. EnterpriseRelationship/ConsolidationDetermination-targeted proposals remain deferred to their own future Business Activities. **Disclosed, not resolved by ADR-007:** whether "Proposed Outcome Context" itself requires its own CBOR registration (SD-002 §2's Cross-Experience Reference Test — the same test that registered SCI-000001 — plausibly applies, since BA-05/BA-06/BA-07 each appear to consume it by name). This reclassification does not authorize implementation; BA-04's own fresh gap analysis, addressing that registration question, is still required per CLAUDE.md §19.7. |
