...
| `role_code`/`role_name` collision with an eventual ADR-002 rename (e.g., if `PLATFORM_ADMIN` is renamed `CORPSTAGE_ADMIN` under ADR-002 Option A) | Low | BA-01 does not seed or rename any row; new roles established via BA-01 use whatever codes their proposing admin supplies, unaffected by a future rename of the pre-existing seed catalog |
