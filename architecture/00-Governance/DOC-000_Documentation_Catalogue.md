...
| TECH-DEBT | Technical Debt Register | Governance | `architecture/06-Reviews/TECH-DEBT.md` | Repository-wide (per-entry ownership) | Governance Registry | Active | N/A (living register, 114 entries — recounted directly against the Register table this pass) | 2026-08-02 (latest: TD-114, disclosing an identical-shape information-disclosure gap found in the already-CLOSED WP-08, incidentally by the WP-09 Gate 4 Independent Verification of Remediation; WP-08 not reopened per `CLAUDE.md §20.1`) | All engineering roles |
| ADR Index | Architecture Decision Records (ADR-001 through ADR-017) | Governance | `architecture/07-Decisions/ADR-*.md` | Repository Owner (per-decision) | Governance (superseding-change authority) | 17 Accepted | N/A (index of 17 documents) | 2026-07-31 (latest: ADR-017) | Architects, Reviewers |
| IRA Reports | Implementation Readiness Assessments (IRA-001, IRA-001A, IRA-002 through IRA-008, IRA-RTA-001, IRA-009) | Governance | `architecture/05-Implementation/IRA-*.md` | Implementing session per WP | Governance (readiness gate) | 11 Accepted (`IRA-009` — WP-09/C-008, accepted 2026-08-01 per Governance Consolidation & Transition to WP-09, following consistent review conclusions in `WP-09-BUSINESS-VALUE-ASSESSMENT.md`/`PLATFORM-DEPENDENCY-ASSESSMENT-TIERRESOLVER.md`) | N/A (index of 11 documents) | 2026-08-01 (latest: `IRA-009`, accepted) | Implementers, Reviewers |
| Independent Review / Certification Reports | CERT-WP-01, CERT-WP-01A, CERT-WP-02, CERT-WP-03, CERT-WP-04, CERT-WP-05, CERT-WP-06, CERT-WP-07, CERT-WP-08, CERT-WP-09, CERT-WP-RTA-001 | Governance | `architecture/06-Reviews/CERT-*.md` | Independent (fresh-context) reviewer, per `CLAUDE.md §19.7` | Governance (certification authority) | 11 issued (10 PASS/CERTIFIED WITH OBSERVATIONS, 1 CERTIFIED WITH CONDITIONS — resolved) — CERT-WP-05's own PASS WITH OBSERVATIONS did not survive a subsequent independent V&V audit (`VV-AUDIT-WP-05`, two `CLAUDE.md §19.8.5`-class defects); both remediated and independently re-verified (`VV-AUDIT-WP-05_Remediation_Verification.md`, CONFIRMED WITH OBSERVATIONS) — WP-05 restored to CLOSED — Certified. CERT-WP-06's own V&V Audit and Release Readiness Audit have both since completed — WP-06 is CLOSED — Certified, committed `a82ff87`. CERT-WP-07 has since been followed by its own V&V Audit (`VV-AUDIT-WP-07`) and Release Readiness Audit (`RRA-WP-07`) — WP-07 is CLOSED — Certified, committed `6da647e`. CERT-WP-08 (first Work Package under `CLAUDE.md §20`) has since been followed by its own V&V Audit (`VV-AUDIT-WP-08`) and Release Readiness Audit (`RRA-WP-08`) — WP-08 is CLOSED — Certified, committed `808c06d`. CERT-WP-09's own V&V Audit found one High/`CLAUDE.md §19.8.5`-class defect (a cross-tenant Membership-status disclosure), remediated and independently re-verified (`VV-AUDIT-WP-09_Remediation_Verification.md`), followed by its own Release Readiness Audit (`RRA-WP-09`) — WP-09 is CLOSED — Certified, committed `90544cb`/`6ce9bd3`/`d648150`. | N/A (index of 11 documents) | 2026-08-02 (latest: CERT-WP-09) | Executives, Reviewers, Auditors |
...
| Document ID | Document Name | Category | Repository Path | Owner | Canonical Status | Lifecycle Status | Current Version | Last Updated | Primary Audience |
|---|---|---|---|---|---|---|---|---|---|
| Implementation Reports | IMP-REPORT-WP-01 through WP-09, IMP-REPORT-WP-RTA-001 | Implementation | `architecture/05-Implementation/IMP-REPORT-*.md` | Implementing session per WP | Implementation audit trail (not certification, per `CLAUDE.md §19` Implementation Reporting note) | 10 issued (9 Closed, 1 Certified-conditions-resolved) | N/A (index of 10 documents) | 2026-08-02 (latest: WP-09 CLOSED, committed `90544cb`/`6ce9bd3`/`d648150`) | Implementers, Reviewers |
...
**Total documents registered above: 60** (14 Architecture + 2 Experience-family entries + 6 Engineering + 2 Design + 33 Governance + 3 Implementation entries [the WP-01…09/RTA-001 family row, `IMP-REPORT-RELEASE-A1`, and `IMP-REPORT-RELEASE-A2`] — recounted directly against §8's own rows; the WP-06 Release Readiness Audit, `RRA-WP-06`, first reconciled this line to 45/20 Governance, the `RRA-WP-06` row itself brought it to 46/21, `VV-AUDIT-WP-07`'s own new row brought it to 47/22, `VV-AUDIT-WP-08`'s own new row (added following `VV-AUDIT-WP-05`/`06`/`07`'s own precedent of a dedicated row per V&V Audit, distinct from the family-folded `CERT-WP-*` row) brought it to 48/23, `RRA-WP-08`'s own Gate 5 review found `RRA-WP-07`'s own row had never been added (a pre-existing omission, not introduced by that pass) — adding it (49/24) and its own `RRA-WP-08` row (50/25) brought the total to 50/25, `PRODUCT-MILESTONE-ROADMAP` (51/26) was added per a separate repository-owner instruction to register that document as a canonical planning artifact, `IMP-REPORT-RELEASE-A1` (52/26 Governance, 2 Implementation) was added per Release A1's own Governance Synchronization phase, `RELEASE-A2-AI-CONFIGURATION-GOVERNANCE-REVIEW` (53/27) was added as a Governance-category advisory review artifact, `AI-CONFIGURATION-TRACEABILITY-MATRIX` (54/28) was added per that validation's own output instruction, `IMP-REPORT-RELEASE-A2` (55/28 Governance, 3 Implementation) was added as a third, distinct Implementation-category entry per Release A2's own closure, `WP-09-BUSINESS-VALUE-ASSESSMENT` (56/29) was added as a further Governance-category advisory review artifact, `PLATFORM-DEPENDENCY-ASSESSMENT-TIERRESOLVER` (57/30) was added as a further Governance-category advisory review artifact, and this pass — WP-09's own Gate 1/2/4/5 closure — adds three further Governance-category rows (`VV-AUDIT-WP-09`, `VV-AUDIT-WP-09_Remediation_Verification`, `RRA-WP-09`; `CERT-WP-09` folded into the existing `CERT-*` family row, `IMP-REPORT-WP-09` folded into the existing Implementation Reports family row, per every prior WP's own precedent) bringing the total to 60/33 Governance — see §12 for the exact count reconciliation).
...
| Active Documents (Status: Active/AUTHORITATIVE/RELEASED/LOCKED, currently governing) | Not re-derived this pass; denominator is 53. The numerator was already left undetermined by `RRA-WP-06`'s own recount (at 45) and is not guessed forward here for the further documents added since — this remains an arithmetic reconciliation, not a full content/canonical-status review |
