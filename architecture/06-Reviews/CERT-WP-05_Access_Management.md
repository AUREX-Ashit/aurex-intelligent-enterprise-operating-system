...
> **ADDENDUM (2026-07-31) — This certification's PASS WITH OBSERVATIONS determination did not survive independent re-verification.**
>
> A subsequent, more rigorous Verification & Validation audit (`VV-AUDIT-WP-05_Access_Management.md`), performed by a second independent, fresh-context reviewer with no involvement in this certification, found **two High-severity defects this certification did not identify**, both falling inside `CLAUDE.md §19.8.5`'s non-deferrable list: **F-01** (BA-01's UNRESOLVED branch wrote a foreign key referencing a nonexistent Membership — passed only because the test harness runs with FK enforcement off; fails with HTTP 500 on PostgreSQL) and **F-02** (the Deferred-branch Approval Authority lookup was not organization-scoped, permitting a cross-tenant information disclosure — demonstrated empirically). Both defects were remediated by the implementing session and **independently re-verified by a third, fresh-context reviewer** (`VV-AUDIT-WP-05_Remediation_Verification.md`, CONFIRMED WITH OBSERVATIONS) — WP-05's `CLOSED — CERTIFIED` status has since been restored (`IMP-REPORT-WP-05_Access_Management.md`, "Correction" section).
>
> This document is preserved **unedited below** as the historical record of the original certification pass — including its now-superseded test-count figures (598/11, since corrected to 601/14 and then 608/19 by subsequent work) and its "None yet" self-certification-prohibition statement about future review — per this repository's own audit-trail discipline (`CLAUDE.md §19.8`: technical debt and process findings are recorded, not retroactively erased from prior documents). Do not treat any claim below as current without cross-checking `VV-AUDIT-WP-05` and `IMP-REPORT-WP-05`'s Correction section first.
